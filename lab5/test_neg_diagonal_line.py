...
# Test script for detecting a a diagonal line with a negative theta angle
...
# using normal form: theta=-45 (degrees), rho=-quarter diagonal size. This line
# image is what we would get from edge detection as it only has two values,
# edge=255 and non-edge=0.
theta_true = -45
rho_true = - round( 0.25 * math.sqrt( size_x * size_x + size_y * size_y ) )
...
# Write image to file.
image_with_line.save( 'neg_diagonal_line.tif' )
...
# Write image to file.
accumulator_log_image.save( 'neg_diagonal_line_accumulator.tif' )